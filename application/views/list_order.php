...
<a href="#modal_history"><button id="btn-history" disabled="disabled">History</button></a> 
...
<!-- Datatables -->
	...
	<thead>
		<tr>
			...
			<th>Responding Vehicle</th>
			<th>Responding Officer/s</th>
			<th>Escalated To</th>
	...
	<div class="remodal" data-remodal-id="modal" id="modal">
    	...
    	<form method="post" id="update-status-form" action="list_order/update_status" onsubmit="return validate();" >
             ...
             <input type="hidden" name="date" id="date" >
				...
				<option value="Dispatched">Dispatched</option>
				<option value="Feedback">Feedback</option>
				<option id="escalation" value="Escalated">Escalated</option>
		...
		<div id="ARcontainer">
        		<label for="ar_num">AR number</label>
			<input type="text" name="ar_num" id="ar_num" >
			...
			<br>
			<label id="lbl_car_data" for="string_car_data">Enter Vehicle Call Sign</label>
			<input type="text" id="string_car_data1" name="string_car_data1" >
			<br>
        		<label for="dispatch_officer">Officer's Pay No</label>
        		<input type="text" id="dispatch_officer1" name="dispatch_officer1" >
...
//$('[data-remodal-id=modal_activity').remodal();
	...
	getRequest("list_order/get_regions", function(data) 
	{
		...
		$('#menu_region_id option[value=0]').attr('selected','selected');
...
$("#menu_channel_id_recapture").html($("#menu_channel_id_recapture option").sort(function (a, b) {
if(a.text != 'Select Channel'){
    return a.text == b.text ? 0 : a.text < b.text ? -1 : 1
}
}))
$('#menu_channel_id_recapture option[value=0]').attr('selected','selected');
...
$("#menu_type_id").empty().append( my_options );
$("#menu_type_id").val(selected);
...
//append type other
$("#menu_type_id").append("<option value=Other>Other</option>");
		        				...
		        				return false;
	         					...
	         					if (menu_hiddenchannel== null || menu_hiddenchannel== "") {
									...
									if (loc== null || loc== "") {
										...
										if (menu_region== null || menu_region== "") {
				        					alert("region must be filled out");
				        					return false;
				    						}else{
...
//Getting group id for directors
getRequest("list_order/get_group_session", function(data) {
...
getRequest("list_order/get_managers/" + escalation_id[0].escalation_id, function(data) {
		
	var data = JSON.parse(data.responseText);

    $("#supervisor_name").append("<option> Select Supervisor (Name) </option>");
    for (var i = 0; i < data.length; i++) {
...
$("ol").append("</br>");
	...
	var oTable = $('#tabels').dataTable({
		...
		"bServerSide": true,
		"iDisplayLength": 50,
		"sAjaxSource": (filter === null ? "list_order/get" : "list_order/get_corruption"),
    ...
    var iPos = oTable.fnGetPosition( nRow );
     //var iPos = oTableExport.fnGetPosition( nRow );
    ...
    if ( iPos != null )
    {
       ...
       //   var aData = oTableExport.fnGetData( iPos );
...
//*************************
	...
	if(aData != null){
		...
		$('#update_id').val(aData[22]);
		$('#orders_id').val(aData[22]);
		//getting the ids
		$('#category_id').val(aData[22]);
		$('#remove_list_id').val(aData[22]);
		update_id = aData[22];
		cat_id = aData[22];
...
var accident_disc  = aData[9];
if(aData[2] == 'Accident')
{

accident_disc = "AR number: " + aData[19]  + "\r\nCar registration number: " + aData[20] + "\r\nNumber of person(s) involved: " +aData[21] + "\r\n \r\n " + aData[9];

}


		$('#description1').val(accident_disc);
